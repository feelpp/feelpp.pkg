...
for i in $rep/*.deb $rep/*.changes; do
    ...
    filename=$(basename $i)
    curl -XPUT -T $i -uchristophe.prudhomme@cemosis.fr:${BINTRAY_API_KEY} "https://feelpp.jfrog.io/artifactory/$repo/pool/$distribution/$filename;deb.distribution=$distribution;deb.component=$channel;deb.architecture=$arch;publish=1;override=1"
